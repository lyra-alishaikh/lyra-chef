...
export default function DuplicateTool() {
  ...
  const dedupe = () => {
    ...
    const unique = [...new Set(lines.map(l => l.trim()).filter(l => l))]
    setOutput(unique.join('\n'))
  }
  return <div><textarea value={input} onChange={e=>setInput(e.target.value)} className="w-full h-48 border rounded-2xl p-5 mb-4" /><button onClick={dedupe} className="w-full py-3 bg-zinc-800 text-white rounded-2xl">Remove Duplicate Lines</button>{output && <pre className="mt-4 p-5 bg-zinc-50 border rounded-2xl text-sm">{output}</pre>}</div>
}